...
import {
  ...
  Zap, HelpCircle, ArrowUpRight, CheckCircle2, ChevronRight
} from 'lucide-react';
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
            ...
            <div className="md:col-span-2 space-y-2">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Contract A (Source / Reference)</label>
              <select
                value={sourceId}
                onChange={(e) => setSourceId(e.target.value)}
                className="w-full px-3 py-3 text-xs font-semibold rounded-xl bg-white/[0.03] border border-white/10 text-white outline-none cursor-pointer"
              >
                {contracts.map((c) => (
            ...
            <div className="md:col-span-1 flex justify-center pt-6">
              <div className="h-10 w-10 rounded-full bg-white/[0.03] border border-white/5 flex items-center justify-center text-gray-500">
            ...
            {/* Target */}
            <div className="md:col-span-2 space-y-2">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Contract B (Target / Proposed)</label>
              <select
                value={targetId}
          ...
          <button
            ...
            disabled={comparing}
            className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 transition-all cursor-pointer"
          >
            {comparing ? (
              <>
          ...
          {comparison && !comparing && (
            <div className="space-y-8 animate-fadeIn">
              ...
              <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 space-y-2">
                <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider block">Comparative Summary</span>
                <p className="text-sm text-gray-300 leading-relaxed">{comparison.summary}</p>
              </div>

              {/* Clause details list */}
              <div className="space-y-6">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider">Clause Diff Breakdown</h3>
                  ...
                  {comparison.comparisons.map((c: any, idx: number) => (
                    <div key={idx} className="glass-card rounded-2xl border-white/5 p-6 flex flex-col space-y-4 relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-center pl-2">
                        <span className="text-xs font-bold text-white uppercase tracking-wider">{c.category} Clause</span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pl-2">
                        ...
                        <div className="space-y-1">
                          <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Reference (Contract A)</span>
                        ...
                        {/* Proposed Clause */}
                        <div className="space-y-1">
                          <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Proposed (Contract B)</span>
                          <p className="text-xs text-gray-400 leading-relaxed bg-white/[0.01] p-3 rounded-lg border border-white/5">
                            {c.contractBText}
                          </p>
                        </div>
                      ...
                      {/* Diff Explanation */}
                      <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 text-xs text-gray-400 leading-relaxed pl-4 ml-2">
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold block mb-1">Difference Analysis</span>
                        {c.difference}
                      </div>
