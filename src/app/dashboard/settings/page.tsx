        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="glass-card rounded-2xl p-6 border-white/5 space-y-6">
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
              ...
              <span className="text-[8px] bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded uppercase font-bold tracking-widest">REST API</span>
            </div>
            ...
            <form onSubmit={handleGenerateApiKey} className="flex gap-4">
              ...
              <button
                type="submit"
                disabled={generating}
                className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white text-xs font-semibold rounded-xl transition-all flex items-center gap-2 cursor-pointer hover:scale-[1.02]"
              >
          ...
          <div className="glass-card rounded-2xl p-6 border-white/5 space-y-4">
            ...
            <div className="bg-black/85 rounded-xl p-4 font-mono text-[10px] leading-relaxed text-gray-400 max-h-[300px] overflow-y-auto space-y-2 border border-white/5">
              {auditLogs.length === 0 ? (
                ...
                auditLogs.map((log) => (
                  <div key={log.id} className="flex gap-2">
                    ...
                    <span className="text-gray-600 ml-auto font-bold font-display uppercase text-[8px] bg-white/5 px-1.5 py-0.5 rounded">
                      {log.user}
                    </span>
                  </div>
                ))
          ...
          {user && (
            <div className="glass-card rounded-2xl p-6 border-white/5 space-y-4">
              ...
              <div className="flex items-center gap-3 bg-white/[0.01] p-3 rounded-xl border border-white/5">
                <div className="h-10 w-10 rounded-xl bg-blue-600/10 border border-blue-500/20 flex items-center justify-center text-blue-400 font-bold font-display text-sm">
                  {user.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h4 className="text-sm font-bold text-white leading-tight font-display">{user.name}</h4>
                  <span className="text-[9px] text-gray-500 font-mono flex items-center gap-1 mt-1"><Mail className="w-3.5 h-3.5 text-gray-600" /> {user.email}</span>
                </div>
              </div>
              ...
              <div className="border-t border-white/5 pt-4 text-[9px] text-gray-500 font-mono space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white truncate font-semibold">{user.organization?.name}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span>Subscription Tier:</span>
                  <span className="text-emerald-400 font-semibold uppercase tracking-wider text-[8px] bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded">Enterprise OS</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 w-[12%] rounded-full animate-pulse" />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-indigo-500 w-[2%] rounded-full animate-pulse" />
                </div>
              </div>
            </div>
          </div>
